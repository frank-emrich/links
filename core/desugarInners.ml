...
let rec add_extras in_fun tyvars extras tyargs =
  ...
  | tv::tvs, None::extras ->
     let q = if in_fun then
               Sugartypes.quantifier_of_tyvar tv
               |> Types.type_arg_of_quantifier
             else
               let open CommonTypes.PrimaryKind in
               match Sugartypes.quantifier_of_tyvar tv with
     ...
     in q :: add_extras in_fun tvs extras tyargs
...
object (o : 'self_type)
  ...
  val extra_env : (Sugartypes.tyvar list * int option list) StringMap.t = StringMap.empty
